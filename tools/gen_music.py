...
def _elevenlabs_music(api_key, prompt, length_ms, model, force_instrumental, output_format):
    body = {
        "prompt": prompt,
...
PROVIDERS = {"elevenlabs": _elevenlabs_music}
...
def main():
    ...
    provider_name = (env.get("MUSIC_PROVIDER") or "elevenlabs").strip()
    provider_fn = PROVIDERS.get(provider_name)
    if todo and provider_fn is None:
        sys.exit(f"Unknown MUSIC_PROVIDER '{provider_name}' — implement a function with the same "
                 "signature as _elevenlabs_music(api_key, prompt, length_ms, model, force_instrumental, "
                 "output_format) and add it to PROVIDERS in tools/gen_music.py.")

    for s, rel, path, length_ms in todo:
        print(f"\n-> {s['id']}  ({length_ms/1000:.0f}s)")
        try:
            audio = provider_fn(api_key, s["prompt"], length_ms, model, force_instrumental, output_format)
