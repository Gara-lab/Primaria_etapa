...
- **For a video** — Master (example): `videos/video-1/reference/<cut>.mp4` · edited transcript:
  `videos/video-1/work/edited-transcript.json` (word times in the master timeline). Shot source:
  `remotion/src/shots/video-N/*.tsx`.
- **For a short** — Artifact root: `shorts/short-N-<niche>/` (`script.md`, `beats.json` — the VO +
  word-time contract, `voice/`, `sfx-plan.json`, `output/`). Shot source:
  `remotion/src/shots/short-N/*.tsx` (the composition, registered by `npm run gen`) plus an
  AUTO-GENERATED `vo.gen.ts` (never hand-edited).
- Reusable brand assets (shared by both video and short pipelines): `media/library/` (typed folders —
  `logos/ sfx/ music/ faces/` each with an index/catalog). Per-project generated assets live at
  `media/projects/<project>/`, where `<project>` is a video's slug (`video-1`) or a short's slug
  (`short-7-kids`), referenced from shots as `staticFile('projects/<project>/x')`.
